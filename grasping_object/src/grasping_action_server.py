...
class GraspingActionServer(ManipulateArm):
    def __init__(self):
        ...
        self.navigation_place = 'Null'   # Navigation成功後のロケーションネーム格納用変数
        ...
        self.act = actionlib.SimpleActionServer('/manipulation/grasp',         # アクションサーバ定義
                                                ...
                                                execute_cb = self.actionMain,  # コールバックメソッドを指定
                                                auto_start = False)            # サーバの自動起動を無効にするためにFalseを指定
        self.act.register_preempt_callback(self.actionPreempt)                 # 実行中のプログラムを強制的に一時中断し、self.actionPreemptを実行する

        self.act.start()  # アクションサーバ起動

    def placeMode(self):  # override(親クラスのメソッドを子クラスで再定義)
    ...
    # オブジェクト(把持対象)に接近するメソッド
    def approachObject(self,object_centroid):  # 引数はオブジェクトの重心の座標 
        if object_centroid.x > 1.5:   # オブジェクトまでの距離が1.5[m]より遠かったら真
            return False
        elif object_centroid.x < 0.5 or object_centroid.x > 0.8:  # オブジェクトまでの距離が x < 0.5 か 0.8 < x だったらTrue
            move_range = (object_centroid.x-0.65)  # [1] オブジェクトに接近する距離（以下、接近距離) > [x < 0.5は後退], [x > 0.8は前進]
            if abs(move_range) < 0.2:  # 接近距離の絶対値が0.2[m]未満なら真
                move_range = int(move_range/abs(move_range))*0.2  # [1]で前進か後退か決まる
            self.base_control.translateDist(move_range)  # 前後退
            rospy.sleep(4.0)
            return False
        else :
            return True
    ...
    # 物体把持をするメソッド
    def graspObject(self, object_centroid):  # 引数はオブジェクトの3次元位置
        rospy.loginfo('\n----- Grasp Object -----')
        ...
        if numpy.nan in joint_angle:  # 非数か確認
            return False
        self.armControllerByTopic(joint_angle)  # 物体把持のマニピュレーション
        rospy.sleep(2.5)
        ...
        grasp_flg = self.controlEndeffector(True)  # エンドエフェクタを閉じる(オブジェクトを掴めたらTrue)
        rospy.sleep(1.0)
        self.controlWrist(joint_angle[2]+45.0)  # 肘を上方向に曲げる
        ...
        if grasp_flg :
            ...
            self.setPosition(4, self.origin_angle[4])  # eefをOPEN
            rospy.loginfo('Failed to grasp the object.')
        rospy.loginfo('Finish grasp.')
        return grasp_flg

    def navigationPlaceCB(self,res):
        self.navigation_place = res.data  # Navigation成功後のロケーションネーム

    # ロボットせっとアップ時に首をまっすぐにし、carryモードにするメソッド
    def startUp(self):
    ...
    def actionPreempt(self):
        ...
        self.act.set_preempted(text = 'message for preempt')  # 強制終了（リクエストがあれば中断）
    ...
    def actionMain(self,object_centroid):  # 引数はアクション通信の引数 *詳しくは /happymimi_manipulation_msgs/action/GraspingObject.action
        # set up
        target_centroid = object_centroid.goal  # アクション通信のゴールを代入(3次元位置推定値)
        grasp_result = GraspingObjectResult()  # アクション通信の結果で初期化
        grasp_flg = False  # 物体把持が成功か否かを格納する変数
        # execute
        approach_flg = self.approachObject(target_centroid)  # オブジェクトに接近. 引数は3次元位置推定値
        if approach_flg:  # オブジェクトへの接近が成功したら真
